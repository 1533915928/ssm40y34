    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="用户评分" prop="yonghupingfen">
          <el-select :disabled="ro.yonghupingfen" v-model="ruleForm.yonghupingfen" placeholder="请选择用户评分">
            <el-option
      ...
      <el-col :span="24">  
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.tianjiatupian" label="添加图片" prop="tianjiatupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.tianjiatupian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
      ...
      this.$http({
		...
		if(json.lianxidianhua!=''&&json.lianxidianhua){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/tuangoushangpin/shangpinmingcheng`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
    ...
    shangpinmingchengChange () {
      this.$http({
        url: `follow/tuangoushangpin/shangpinmingcheng?columnValue=`+ this.ruleForm.shangpinmingcheng,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	if(this.ruleForm.tianjiatupian!=null) {
		this.ruleForm.tianjiatupian = this.ruleForm.tianjiatupian.replace(new RegExp(this.$base.url,"g"),"");
	}
